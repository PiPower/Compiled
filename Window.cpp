...
Window::Window(int width, int height)
	:Hinstace(GetModuleHandle(nullptr)) , width(width),height(height)
{
...
{
	Dx11 = new Graphics(hwnd,height, width);
	Dx11Init = true;
}
...
Window::~Window()
{
	...
	UnregisterClass(CLASS_NAME.c_str(), Hinstace);
...
LRESULT Window::HandleMsg(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) 
{
	...
	case WM_SIZE:
		...
		//if (Dx11Init) Dx11->Resize(hwnd, width,height);
		break;
	}
...
Window::Window(int width, int height, HWND Parent)
	:
 Hinstace(GetModuleHandle(nullptr)), width(width), height(height)
{
	CLASS_NAME = L"Child";
	...
	DWORD style = WS_BORDER | WS_CAPTION | WS_SYSMENU | WS_EX_CLIENTEDGE;
	...
	hwnd = CreateWindowEx(0, CLASS_NAME.c_str(), L"Child",
		style, CW_USEDEFAULT, CW_USEDEFAULT, wr.right - wr.left, wr.bottom - wr.top,
		Parent, nullptr, Hinstace, this);
